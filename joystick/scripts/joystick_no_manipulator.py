...
class XboxNode(object):
    def __init__(self):
        rospy.init_node('xbox_node')

        # Subscribes to joy via a throttler
        self.sub = rospy.Subscriber(
            'joy_throttle', Joy, self.callback, queue_size=1)
        
        # Publishes general motion data that will be interpreted in the controller
        self.pub_motion = rospy.Publisher('propulsion_command',
                                          PropulsionCommand,
                                          queue_size=1)

        # Buttons and axes are given names based on joy-configuration
